...
\begin{lemma}[The Sequence Lemma]\label{2.3.5}
    ...
    $A$ converging to $x \in X$, then $x \in \bar{A}$. The converse holds if $X$ is metrizable.
\end{lemma}
\begin{proof}
    Suppose for some sequence $\{x_n\} \subseteq A$ that $x_n \rightarrow x$. By theorem \ref
    {1.6.6}, we have every neighborhood of $x$ contains points of  $A$,  hence $x \in \bar{A}$.
    Conversely, suppose that $X$ is metrizable with metric  $d$, and let  $x \in \bar{A}$. For $n
