...
\end{theorem}
\begin{proof}
    Notics that $\rho(x,y) \leq ||x-y|| \leq \sqrt{n}\rho(x,y)$. This first inequality shcows that
...
\begin{definition}
    ...
    define the \textbf {uniform metric} $\hat{\rho}$ on $\R^J$ by
    $\hat{\rho}(x,y)=\sup\{\hat{d}(x_{\alpha},y_{\alpha}): \alpha \in J\}$, where $\hat{d}$ is the
    standar bounded metric on  $\R^J$. We call the topology induced by $\hat{\rho}$ the
...
\begin{theorem}\label{2.2.9}
    Let $\hat{d}(a,b)=\min{|a-b|,1\}$ be the standard bounded metric on $\R$, and for  $x,y \in
    \R^{\omega}$, define:
        \begin{equation}
            D(x,y)=\sup\{\frac{\hat{d}(x_i,y_i)}{i}\}
        \end{equation}.
    Then $D$ is a metric that induces the product topology on $\R^{\omega}$.
\$B_{\hat{\rho}}(x,\epsilon)$
\end{theorem}
\begin{proof}
    Since $\hat{d}$ is a metric, $D$ satisfies the conditions for a metric space, it is worth
    looking into the case for the triangle inequality however. Notice that for all  $i$,  $
    \frac{\hat{d}(x_i,y_i)}{i} \leq \frac{\hat{d}(x_i,z_i)}{i}+\frac{\hat{d}(z_i,y_i)}{i} \leq
    ...
    \R^{\omega}$, we have $ \frac{\hat{d}(x_i,y_i)}{i} \leq \frac{1}{N}$, thus $D(x,y)$ \leq
    \max\{\hat(d)(x_1,y_1),\dots, \hat(d)(x_i,y_i)}{i}, \frac{1}{N}\} for all $i \geq N$. Now if  $y
    ...
    we see that $ \frac{\hat{d}(x_i,y_i)}{i} \leq D(x,y)<\epsilon$. Now if $i=\alpha_1, \dots,
    \alphaalpha_n$, then $\hat{d}(x_i,y_i)<\epsilon_i \leq 1$, hence $|x_i-y_i|<\epsilon_i$, thus
    $y \in U$.
\end{proof}
